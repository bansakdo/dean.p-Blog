...
- Jenkins 전용 작업: Pipeline script from SCM, Git 저장소 `https://github.com/bansakdo/dean.p-Blog.git`, 브랜치 `*/dev`, 스크립트 `Jenkinsfile.dev`. `java25` 에이전트의 단일 executor가 `blog-main`과 CI 전용 DB 테스트를 직렬화합니다. 다른 executor를 추가할 때는 DB 공유 잠금 또는 전용 CI DB가 필요합니다.
- Jenkins Credentials: 기존 `dean-p-blog-ci-db`와 **개발 서버 전용** SSH 개인키 `dean-p-blog-dev-deploy-ssh`(SSH Username with private key, username `macmini`). 공개키는 Mac mini `authorized_keys`에서 별도 `deploy.py` 강제 명령으로 제한합니다. 운영 배포 키를 재사용하지 않습니다.
